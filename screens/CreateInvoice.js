...
export default function CreateInvoice({ route, navigation }) {
    ...
    const [IDBill, setIDBill] = useState(''); // Sử dụng useState để lưu mã hóa đơn mới

    useEffect(() => {
        let noBill = `${currentDate}${houseNow}-${idTemplate}`;
    ...
    const printToFile = async () => {
        if (html1 !== null) {
            const { uri } = await Print.printToFileAsync({ html1 });
